...
class Animanga(commands.Cog):
    ...
    @commands.command(
        usage='<anime [name]',
        brief="Returns info for an anime"
    )
    async def anime(self, ctx, *, args=None):
        if not args:
            return await ctx.reply("You didn't give me a name", mention_author=False)
        try:
            try:
                mid = jikan.search("anime", args).get("results")[0].get("mal_id")
            except APIException:
                return await ctx.send("Error connecting to service, try again with a different name")
            if mid:
    ...
    @commands.command(
        usage='<manga [name]',
        brief="Returns info for a manga"
    )
    async def manga(self, ctx, *, args=None):
        if not args:
            return await ctx.reply("You didn't give me a name", mention_author=False)
        query = " ".join(args)
        try:
            try:
                mid = jikan.search("manga", args).get("results")[0].get("mal_id")
            except APIException:
            ...
            await ctx.reply(f"No results found, try with a diffferent name", mention_author=False)


    @commands.command(
        usage='<character [name]',
        brief="Returns info for animanga character"
    )
    async def character(self, ctx, *, args=None):
        ...
        try:
            try:
                mid = jikan.search("character", args).get("results")[0].get("mal_id")
            except APIException:
    ...
    @commands.command(
        usage='<sauce [numbers]',
        brief="Returns info for sauce"
    )
    async def sauce(self, ctx, arg=None):
        if not arg:
            return await ctx.reply("You didn't give me sauce though", mention_author=False)
        ...
        if ctx.channel.is_nsfw():
            ...
            embed = discord.Embed(
                description="You need to be in a NSFW text channel to use this"
            )
            await ctx.send(embed=embed)
